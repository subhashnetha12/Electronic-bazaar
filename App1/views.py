...
def edit_customer(request, id):
    ...
    if request.method == 'POST':
        try:
            ...
            customer.email = request.POST['email'] or None
            customer.phone_number = request.POST['phone_number'] or None
...
def add_daily_production(request):
    ...
    if request.method == "POST":
        ...
        refurbished_date = request.POST.get("refurbished_date")
        stock_in = request.POST.get("stock_in")
        serial_number = request.POST.get("serial_number")
        sale_price = request.POST.get("sale_price")
        mrp = request.POST.get("mrp")

        if DailyProduction.objects.filter(serial_number=serial_number).exists():
...
def edit_daily_production(request, id):
    ...
    if request.method == 'POST':
        try:
            ...
            new_serial_number = request.POST['serial_number']
            new_refurbished_date = request.POST['refurbished_date']
            new_mrp = float(request.POST['mrp'])
            new_sale_price = float(request.POST['sale_price'])
            new_weight = float(request.POST['weight'])

            if DailyProduction.objects.filter(serial_number=new_serial_number).exclude(id=daily_production.id).exists():
                messages.error(request, "Batch number already exists.")
                return render(request, 'company_admin/add_daily_production.html', {
            ...
            daily_production.serial_number = new_serial_number
            daily_production.sale_price = new_sale_price
            daily_production.mrp = new_mrp
            daily_production.refurbished_date = new_refurbished_date
...
def get_product_details(request, product_id):
    try:
        prod = DailyProduction.objects.filter(product_id=product_id, stock_in__gt=0).order_by('id').first()
        if not prod:
        ...
        return JsonResponse({
            ...
            "mrp": float(prod.mrp),
            "gstpercentage": prod.product.gstpercentage,
